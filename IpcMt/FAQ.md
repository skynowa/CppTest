...
- Чи є С++ thread-safe?
- У чому різниця між багатопотоковістю та асинхронністю?
- Що таке багатопотоковість? Яку функціональність надає С++ для розробки багатопотокових застосунків? Які основні проблеми багатопотокових застосунків?
- Як передати інформацію між кількома процесами?
- Як синхронізувати між собою кілька процесів?
- Які є особливості роботи з shared memory?
- Як працює spinlock?
- Які ви знаєте особливості використання recursive mutex?
- Розкажіть про read-write mutex.
- Що таке race-condition? Взаємне блокування? Що таке критична секція?
- Які уникнути стану гонитви?
- Чим відрізняється м’ютекс від семафора?
- Які примітиви синхронізації реалізовані в C++? Переваги lock_guard?
- Що трапиться, якщо exception вийде за межі потоку? Які інструменти є для безпечної асинхронності в С++?
- Чим відрізняється std::launch::async від std::launch::deferred?
- Що таке атомарна операція? std::atomic?
- Як працювати з std::conditional_variable?
- Як створити потік за допомогою std::thread?
- На скільки потоків найкраще розбити задачу? Від чого це залежить?
- Як працювати з std::async?
- Thread-safe гарантії контейнерів в С++? Чому недолік інтерфейсу front() + pop_fornt()?

## Senior

- Розкажіть про побудову API, розрахованих на багатопотокове використання.
- У чому різниця між kernel-level та user-level потоками?
- Що таке coroutine?
- Що робить специфікатор thread_local?
- Як реалізувати синхронізацію в задачі producer-consumer?
- Як синхронізуватися між різними процесами?
